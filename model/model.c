...
/* Retorna a proxima direcao do fantasma gi usando Dijkstra — O((V+E)log V) */
static int ghost_dijkstra(GameModel *m, int gi) {
    Ghost *g = &m->ghosts[gi];
    int src = coord_to_vertex(g->x, g->y);
    int dst = coord_to_vertex(m->pacman.x, m->pacman.y);
    dijkstra(m->maze_graph, src, m->dijkstra_dist, m->dijkstra_prev);

    /* Reconstroi caminho do destino ate a origem */
    if (m->dijkstra_dist[dst] == INF) return DIR_NONE;
    int path[MAX_VERTICES], len = 0, cur = dst;
    while (cur != -1 && len < MAX_VERTICES) { path[len++] = cur; cur = m->dijkstra_prev[cur]; }
    if (len < 2) return DIR_NONE;
    int nx, ny;
    vertex_to_coord(path[len-2], &nx, &ny);
    int dx = nx - g->x, dy = ny - g->y;
...
/* Retorna a proxima direcao usando BFS — O(V+E) */
static int ghost_bfs(GameModel *m, int gi) {
    ...
    if (m->bfs_dist[dst] == INF) return DIR_NONE;
    int path[MAX_VERTICES], len = 0, cur = dst;
    while (cur != -1 && len < MAX_VERTICES) { path[len++] = cur; cur = m->bfs_prev[cur]; }
    if (len < 2) return DIR_NONE;
    int nx, ny;
    vertex_to_coord(path[len-2], &nx, &ny);
    int dx = nx - g->x, dy = ny - g->y;
...
/* Retorna a proxima direcao usando DFS — O(V+E) */
static int ghost_dfs(GameModel *m, int gi) {
    Ghost *g = &m->ghosts[gi];
    int src = coord_to_vertex(g->x, g->y);
    dfs(m->maze_graph, src, m->dfs_visited, m->dfs_order, &m->dfs_order_len);

    /* Pega o segundo vertice visitado como proximo passo */
    if (m->dfs_order_len < 2) return DIR_NONE;
    int nx, ny;
    vertex_to_coord(m->dfs_order[1], &nx, &ny);
    int dx = nx - g->x, dy = ny - g->y;
    /* Normaliza: DFS pode ir longe, pega so passo adjacente */
    if (dx > 1) dx = 1;
    else if (dx < -1) dx = -1;
    if (dy > 1) dy = 1;
    else if (dy < -1) dy = -1;
...
/* Retorna direcao aleatoria valida */
static int ghost_random(GameModel *m, int gi) {
    ...
    for (int d = 0; d < 4; d++) {
        int nx = g->x + dirs[d][0];
        int ny = g->y + dirs[d][1];
        if (model_is_navigable(m, nx, ny, 1)) valid[vcnt++] = d;
    }
    if (!vcnt) return DIR_NONE;
    int chosen = valid[rand() % vcnt];
    int dx = dirs[chosen][0], dy = dirs[chosen][1];
    if (dx == 1) return DIR_RIGHT;
    if (dx ==-1) return DIR_LEFT;
    if (dy == 1) return DIR_DOWN;
    return DIR_UP;
...
int model_ghost_ai(GameModel *m, int gi) {
    Ghost *g = &m->ghosts[gi];
    if (g->mode == GMODE_FRIGHTENED || g->mode == GMODE_EATEN)
...
static int ghost_house_exit_dir(Ghost *g) {
    if (g->y <= 11) return DIR_NONE;       /* ja saiu do covil */
    /* Alinha na coluna da porta (13 ou 14) e entao sobe */
...
static int resolve_ghost_collision(GameModel *m, int gi) {
    ...
    if (g->mode == GMODE_FRIGHTENED) {
        /* Pac-Man come o fantasma vulneravel */
        m->score += SCORE_GHOST;
